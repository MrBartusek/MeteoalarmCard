...
import { MeteoalarmAlert, MeteoalarmAlertKind, MeteoalarmAlertTiming } from '../types';
...
function formatCurrentCaption(alert: MeteoalarmAlert, now: Date): WarningCaption | undefined {
	...
	return {
		caption: `${formatTimingValue(start!, alert.timing, now)}–${formatTimingValue(end, alert.timing, now)}`,
	};
}
...
function isFullDayTiming(timing: MeteoalarmAlertTiming | undefined): boolean {
	...
	return isSameLocalCalendarDay(start, end) && isStartOfLocalDay(start) && isEndOfFullDay(end);
}
